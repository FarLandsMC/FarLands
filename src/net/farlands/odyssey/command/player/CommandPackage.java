...
public class CommandPackage extends PlayerCommand {
    ...
    public boolean execute(Player sender, String[] args) {
        ...
        if (sender.getUniqueId().equals(recipientFlp.uuid)) {
            TextUtils.sendFormatted(sender, "&(red)You cannot send a package to yourself.");
            return true;
        }
        ...
        if (timeRemaining > 0) {
            TextUtils.sendFormatted(sender, "&(red)You can send this person another package in %0",
        ...
        if (recipientFlp.isOnline()) {
            ...
            TextUtils.sendFormatted(player, "&(gold){&(aqua)%0} has sent you {&(aqua)%1}%2",
        ...
        // If the recipient is not online then queue it for when the log back in
            ...
            if (FarLands.getDataHandler().addPackage(recipientFlp.uuid, senderSession.handle.getDisplayName(), item, message)) {
                ...
                TextUtils.sendFormatted(sender, "&(green)Package sent.");
            }
            // The sender already has a package queued for this person so the transfer failed
            else
                TextUtils.sendFormatted(sender, "&(red)You cannot send %0 a package right now.", recipientFlp.username);
        }
