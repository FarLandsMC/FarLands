...
public final class FileSystem {
    ...
    public static void writeUTF8(String data, File file) throws IOException {
        ...
        ofstream.write(data.getBytes("UTF-8"));
        ofstream.flush();
        ofstream.close();
    }
    ...
    public static <T> T loadJson(Class<T> clazz, File file) {
        try{
            ...
            FarLands.error("Failed to load " + file.getName() + ".");
    ...
    public static <T> void saveJson(T object, File file) {
        try {
            writeUTF8(FarLands.getGson().toJson(object), file);
        }catch(IOException ex) {
            FarLands.error("Failed to save " + file.getName() + ".");
            ex.printStackTrace(System.out);
        }
    }
}
