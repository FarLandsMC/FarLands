...
public class FireworkBuilder {
    ...
    public ItemStack buildItemStack(int stackSize) {
        net.minecraft.server.v1_14_R1.ItemStack stack = CraftItemStack.asNMSCopy(new ItemStack(Material.FIREWORK_ROCKET, stackSize));
        NBTTagCompound stackTag = toNBT().getCompound("FireworksItem").getCompound("tag");
        int flightRaw = (int)Math.ceil(((double)lifetime) / 20.0);
        int flight = flightRaw < 1 ? 1 : (flightRaw > 3 ? 3 : flightRaw);
