...
public class CommandRules extends Command {
    private static final List<Pair<String, String>> RULES = Arrays.asList(
            ...
            new Pair<>("Slurs, hate speech, sexual and/or NSFW (not safe for work) content are prohibited.", "\n- " +
                    ...
                    "portion of our userbase are minors. Gore and other revolting content is also prohibited.\n- " +
                    "Swearing is allowed on the server within reason. If it is directed toward a particular player, " +
                    "you may be asked to stop by the player or a staff member. If you continue, then you may be " +
                    "muted or banned.\n- The discussion of politics, religion, race, and other hot-button topics is " +
                    "not strictly prohibited, however we tend to frown upon such discussion, as it often causes " +
                    "conflict and arguments. If a staff member asks you to take the discussion to PMs or off the " +
                    "server, you must do so or risk getting muted or banned."),
            new Pair<>("Respect other players at all times.", "Brigading servers and/or harassing players will not " +
            ...
            new Pair<>("Bypassing restrictions to harass or grief* another player is prohibited.", "This includes, " +
                    ...
                    "killing, or otherwise bypassing the PvP toggle\n*Staff are not responsible for who you trust on " +
                    "your claim(s). If a player griefs a build or steals items on a claim in which they are trusted, " +
                    "staff will not punish that player, restore the build, or return lost items."),
            new Pair<>("Spam and/or advertisement of other servers or commercial sites is not allowed.", "Types of " +
                    "spam include, but are not limited to:\n- Repeating a particular word, phrase, or character " +
                    ...
                    "(discord only)"),
            new Pair<>("Mechanisms that induce excessive server lag are not allowed.", "This includes large, " +
            ...
            new Pair<>("Respect all staff and staff decisions.", "All players are allowed to appeal a ban, however, " +
                    "openly harassing or ridiculing staff will not be tolerated."),
            new Pair<>("Rules are subject to common sense.", "Any attempt to use loopholes or exploits to violate " +
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length == 0) {
            ...
            for (Pair<String, String> rule : RULES) {
                rules.append("$(hovercmd,/rules ").append(number).append(",{&(gray)View rule ").append(number)
                        .append("},{").append(number).append(". {&(white)").append(rule.getFirst()).append("}})\n");
                ++ number;
            }
            TextUtils.sendFormatted(
                    sender,
                    "&(gold)Server rules (click to view full rule):\n%0",
                    rules.toString().trim()
            );
        } else {
            int rule;
            ...
            TextUtils.sendFormatted(sender, "&(white){&(gold)%0. %1} %2", rule, ruleText.getFirst(), ruleText.getSecond());
        }
